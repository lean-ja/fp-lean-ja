...
`then` ブランチでは、`h` は `n > 0` という根拠に束縛され、この根拠は `Subtype` のコンストラクタの第2引数として使うことができます。
